...
def recursively_compare_objects_type(obj1: Any, obj2: Any) -> bool:
    # recursively compare object types
    ...
    if isinstance(obj1, (tuple, list, set)):
        if len(obj1) != len(obj2):
            return False
        return all(recursively_compare_objects_type(sub_obj1, sub_obj2) for sub_obj1, sub_obj2 in zip(obj1, obj2))

    if isinstance(obj1, dict):
        if len(obj1) != len(obj2):
            return False
        return all(recursively_compare_objects_type(key1, key2) for key1, key2 in zip(obj1.keys(), obj2.keys())) \
               and all(recursively_compare_objects_type(val1, val2) for val1, val2 in zip(obj1.values(), obj2.values()))

    if isinstance(obj1, np.ndarray) and obj1.dtype is object:
        if obj1.shape != obj2.shape:
            return False

        return np.all(np.vectorize(recursively_compare_objects_type)(obj1, obj2))

    if isinstance(obj1, np.ndarray) and obj1.dtype is not object:
        return obj1.dtype is obj2.dtype
