...
def test_shape_only_function_quib_is_fully_invalidated_upon_list_extension_assignment():
    a = iquib([1, 2, 3])
    b = np.zeros_like(a).setp(cache_behavior='on')
    b.get_value()
...
def test_shape_only_function_quib_is_not_invalidated_upon_list_assignment():
    a = iquib([1, 2, 3])
    b = np.zeros_like(a).setp(cache_behavior='on')
    b.get_value()
