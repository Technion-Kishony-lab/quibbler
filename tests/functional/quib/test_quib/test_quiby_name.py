...
def test_quiby_name_invalidates_when_parent_name_changes(a):
    ...
    assert a_name.get_value() == 'a'
    assert a_name.handler.quib_function_call.result_shape is not None
    a.assigned_name = 'new_a'
    assert a_name.handler.quib_function_call.result_shape is None
...
def test_quiby_name_invalidates_when_grand_grand_parent_name_changes(a, c):
    ...
    assert c_name.handler.quib_function_call.result_shape is not None
    a.assigned_name = 'new_a'
    assert c_name.handler.quib_function_call.result_shape is None
